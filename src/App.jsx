    ...
    <div className="bg-gray-100 min-h-screen">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          {news.length > 0 && (
            <div className="md:col-span-2 relative bg-gray-100 rounded-lg overflow-hidden shadow-md">
              ...
              <div className="absolute bottom-0 bg-black bg-opacity-60 text-white p-6 w-full">
                <p className="text-xs bg-teal-200 inline-block px-2 py-1 text-teal-800 font-semibold rounded mb-2">
                  {news[0].category}
                </p>
                <p className="text-xs mb-1">{news[0].date}</p>
                <h3 className="text-2xl font-bold leading-tight">{news[0].title}</h3>
                <p className="mt-2 text-sm line-clamp-3">{news[0].description}</p>
                <button className="mt-4 text-teal-400 font-semibold hover:text-teal-300 transition">
                  → Read now
                </button>
              </div>
            </div>
          ...
          <div className="space-y-4">
            {news.slice(1).map((item, index) => (
              <div
                key={index}
                className="bg-teal-50 p-4 rounded-lg shadow-sm hover:bg-teal-100 hover:shadow-md transition duration-200"
              >
                <p className="text-xs bg-teal-200 inline-block px-2 py-1 text-teal-800 font-semibold rounded mb-1">
                  {item.category}
                </p>
                <p className="text-xs text-gray-600 mb-1">{item.date}</p>
                <h4 className="text-base font-semibold text-teal-800 leading-tight">{item.title}</h4>
                <button className="mt-2 text-teal-600 font-semibold text-sm hover:text-teal-500 transition">
                  → Read now
                </button>
              </div>
            ))}
      ...
      <div id="about" className="container mx-auto py-8">
        ...
        <p className="text-gray-700 text-center">
          Road2Jannah Foundation is dedicated to glorifying Allah (SWT) by uplifting vulnerable communities through
          charity and empowerment. Guided by Islamic values, we focus on orphans, widows, and rural families,
          providing sustainable support to help them thrive. Our mission aligns with the Hadith: “The best among you
          are those who bring greatest benefits to others.”
        </p>
      </div>
      <div id="donate" className="container mx-auto py-8">
        ...
        <p className="text-gray-700 text-center mb-6">
          Your generous contributions help us provide food, empowerment, and care to those in need. Join us in
          fulfilling the spirit of Sadaqah and Fidyah, making a lasting impact.
        </p>
        <div className="text-center">
          <a href="https://www.example.com/donate" className="bg-teal-800 text-white py-2 px-6 rounded-lg hover:bg-teal-700">
            Make a Donation
          </a>
        </div>
      </div>
      <div id="contact" className="container mx-auto py-8">
        ...
        <div className="flex justify-center space-x-4 mt-4">
          <a href="https://facebook.com/road2jannah" target="_blank" className="text-teal-800 hover:underline">
            Facebook
          </a>
          <a href="https://twitter.com/road2jannah" target="_blank" className="text-teal-800 hover:underline">
            Twitter
          </a>
          <a href="https://instagram.com/road2jannah" target="_blank" className="text-teal-800 hover:underline">
            Instagram
          </a>
        </div>
      </div>
